...
class ZerothOrderOptimizationConstrainedAttackTargeted:
    _attackName = "ZerothOrderOptimizationConstrainedTargeted"
    def execute(self, modeldata: ModelData) -> PerturbedData:
        ...
        try:
            ...
            attack = ZooAttack(classifier=model, targeted=True, verbose=True, **config)
            start = time.time()
